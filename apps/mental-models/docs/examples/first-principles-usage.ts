...
async function businessOptimizationExample(): Promise<void> {
  try {
    ...
    console.log('-------------------');
    console.log(
      `Wickedness Score: ${result.wickednessScore.toFixed(2)} (${result.recommendedApproach})`
    );
...
async function technicalProblemExample(): Promise<void> {
  try {
    ...
    const metrics = result.components.filter(
      (c: string) => c.includes('[metrics]') || c.includes('[temporals]')
    );
...
async function wickedProblemExample(): Promise<void> {
  try {
    ...
    const result = await model.analyze({
      problem:
        'How should we balance economic growth with environmental sustainability given conflicting stakeholder interests and uncertain long-term outcomes?',
    });
...
async function qualityMetricsExample(): Promise<void> {
  try {
    ...
    const result = await model.analyze({
      problem:
        'How can we increase revenue by 25% while maintaining 80% gross margins and staying within our $500k marketing budget?',
    });
...
async function batchProcessingExample(): Promise<void> {
  try {
    ...
    const results = await Promise.all(problems.map((problem) => model.analyze({ problem })));
    ...
    console.log(
      `Total Execution Time: ${results.reduce((sum, r) => sum + r.metadata.executionTime, 0)}ms`
    );
    console.log('\n');
  } catch (error: unknown) {
    if (error instanceof Error) {
