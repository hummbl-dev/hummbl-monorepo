...
import { createSY14Model } from '../index';
...
describe('SY14', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy14');
    expect(model.name).toBe('SY14 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY14Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY14Model({
