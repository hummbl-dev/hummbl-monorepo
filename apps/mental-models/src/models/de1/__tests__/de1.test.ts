...
describe('DE1: Structural Decomposition Model', () => {
  ...
  describe('Complex System Decomposition', () => {
    it('should handle complex nested systems', () => {
      ...
      expect(result.components).toContainEqual(expect.objectContaining({ name: 'Frontend' }));

      // Should have nested components
      expect(result.components).toContainEqual(expect.objectContaining({ name: 'UI Framework' }));
