...
import { createSY09Model } from '../index';
...
describe('SY09', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy09');
    expect(model.name).toBe('SY09 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY09Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY09Model({
