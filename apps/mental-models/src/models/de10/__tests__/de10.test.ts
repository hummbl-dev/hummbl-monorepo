...
import { createDE10Model } from '../index';
...
describe('DE10', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('DE10');
    expect(model.name).toBe('DE10');
  ...
  it('should handle errors for invalid input', async () => {
    ...
    await expect(model.analyze({ system: null })).rejects.toThrow(
      'must contain a "system" property'
    );
  });
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createDE10Model({
