...
export function createCognitiveTracingModel(config: Partial<CognitiveTracingModel['config']> = {}) {
  const model: CognitiveTracingModel = {
    ...
    description:
      'Traces reasoning paths through decomposed cognitive steps with annotated decision points',
    ...
    async traceSteps(params: TraceCognitiveStepsParams): Promise<CognitiveTraceResult> {
      ...
      const processStep = async (
        ...
        if (currentDepth >= maxDepth) {
          trace.steps.push(
            createCognitiveStep('Maximum depth reached', 'reflection', [currentStep.id], {
        ...
        if (trace.steps.length >= DE2_CONSTANTS.DEFAULTS.MAX_STEPS_PER_TRACE) {
          trace.steps.push(
            createCognitiveStep('Maximum steps reached', 'reflection', [currentStep.id], {
        ...
        if (Date.now() - startTime > this.config.maxTraceDurationMs) {
          trace.steps.push(
            createCognitiveStep('Time limit exceeded', 'reflection', [currentStep.id], {
      ...
      const averageConfidence =
        trace.steps.reduce((sum, step) => sum + step.confidence, 0) / totalSteps;
    ...
    // Helper methods
    calculateMaxDepth(trace: CognitiveTrace): number {
      ...
      const rootSteps = trace.steps.filter(
        (step) => step.parentIds.length === 0 || step.parentIds.every((pid) => !stepMap.has(pid))
      );
    ...
    calculateMaxBreadth(trace: CognitiveTrace): number {
    ...
    async generateNextSteps(
      ...
      for (let i = 0; i < numSteps; i++) {
        ...
        const step = createCognitiveStep(content, type, [currentStep.id], {
          tags: [type],
