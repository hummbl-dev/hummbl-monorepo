...
import { createSY19Model } from '../index';
...
describe('SY19', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy19');
    expect(model.name).toBe('SY19 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY19Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY19Model({
