...
import { createSY06Model } from '../index';
...
describe('SY06', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy06');
    expect(model.name).toBe('SY06 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY06Model({
      options: {
    ...
    expect(
      result.insights.some(
        (insight) => typeof insight === 'string' && insight.includes('Low confidence result')
      )
    ).toBe(true);
  ...
  it('should handle errors for invalid input', async () => {
    ...
    await expect(model.synthesize({ models: [] })).rejects.toThrow(
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY06Model({
