...
describe('P17', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP17Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p17');
    expect(defaultModel.name).toBe('P17 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP17Model({
