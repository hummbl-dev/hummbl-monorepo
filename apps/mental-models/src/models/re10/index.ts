 ...
 */
export const createRE10Model = (config: Partial<ReconstructionConfig> = {}) => {
  ...
  const reconstruct = async (input: ReconstructionInput): Promise<ReconstructionOutput> => {
    ...
    try {
      ...
      if (input.strategy === 'optimize') {
        ...
        if (
          reconstructedSystem.relationships.length === 0 &&
          reconstructedSystem.components.length > 1
        ) {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Unknown error during reconstruction';
...
export default createRE10Model;
