...
describe('P13', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP13Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p13');
    expect(defaultModel.name).toBe('P13 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP13Model({
