...
export class FirstPrinciplesModelImpl implements FirstPrinciplesModel {
  ...
  public on(event: 'analysisStart', listener: (input: FirstPrinciplesInput) => void): this;
  public on(event: 'analysisComplete', listener: (output: FirstPrinciplesOutput) => void): this;
  ...
  public async analyze(input: unknown): Promise<FirstPrinciplesOutput> {
    ...
    try {
      ...
      this.requestCount++;
      this.eventEmitter.emit('analysisStart', { ...validatedInput, metadata: { ...validatedInput.metadata, requestId } });
      ...
      this.eventEmitter.emit('error', errorToEmit);
