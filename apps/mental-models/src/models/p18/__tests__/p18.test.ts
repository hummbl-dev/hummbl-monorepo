...
describe('P18', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP18Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p18');
    expect(defaultModel.name).toBe('P18 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP18Model({
