...
import { SY2_CONSTANTS, createSchemaMapping, createSchemaMappingResult } from './constants';
import type {
  SchemaMapping,
...
export function createSchemaMapper(config: Partial<SchemaMapper> = {}): SchemaMapper {
  const model: SchemaMapper = {
    ...
    transformValue(value: any, transformFn: string | ((value: any) => any)): any {
      try {
        if (typeof transformFn === 'string') {
          const fn =
            SY2_CONSTANTS.TRANSFORMATIONS[
              transformFn as keyof typeof SY2_CONSTANTS.TRANSFORMATIONS
            ];
    ...
    getValueByPath(obj: Record<string, any>, path: string[], defaultValue?: any): any {
    ...
    calculateConfidence(result: SchemaMappingResult): number {
      ...
      if (typeof sourceSchema === 'object' && typeof targetSchema === 'object') {
        for (const [targetKey, targetValue] of Object.entries(targetSchema)) {
          ...
          if (sourceKey) {
            ...
            if (
              typeof sourceSchema[sourceKey] === 'object' &&
    ...
    fieldNameSimilarity(a: string, b: string): number {
      ...
      for (const [abbr, expansions] of Object.entries(abbreviations)) {
        if (
          (str1 === abbr && expansions.includes(str2)) ||
