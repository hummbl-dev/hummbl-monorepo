...
import { createCO14Model } from '../index';
...
describe('CO14', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('CO14');
    expect(model.name).toBe('CO14');
  ...
  it('should handle errors for invalid input', async () => {
    ...
    await expect(model.analyze({ elements: [] })).rejects.toThrow(
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createCO14Model({
