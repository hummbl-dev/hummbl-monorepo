...
import { createSY05Model } from '../index';
...
describe('SY05', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy05');
    expect(model.name).toBe('SY05 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY05Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY05Model({
