...
describe('P12', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP12Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p12');
    expect(defaultModel.name).toBe('P12 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP12Model({
