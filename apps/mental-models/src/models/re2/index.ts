...
export async function recursiveDecompose<T, R>(
  ...
  try {
    ...
    await decomposeAndSolve(decomposition, config, emitEvent);

    // Get the final solution
    const rootSubproblem = Array.from(decomposition.subproblems.values()).find(
      (sub) => sub.parentId === null
    );

    if (
      !rootSubproblem ||
      rootSubproblem.status !== 'solved' ||
      rootSubproblem.solution === undefined
    ) {
...
async function decomposeAndSolve<T, R>(
  ...
  if (isBase) {
    ...
    try {
      ...
      decomposition = updateSubproblem(decomposition, subproblem.id, {
        ...
        meta: {
          ...
          durationMs:
            new Date().getTime() - (subproblem.meta.startedAt?.getTime() || new Date().getTime()),
        },
      });
    ...
    for (const subproblemData of subproblems) {
      ...
      const childSubproblem = createSubproblem<T, R>(subproblemData, subproblem.id, depth + 1);
      ...
      await decomposeAndSolve(decomposition, config, emitEvent, childSubproblem.id, depth + 1);
    }
    ...
    try {
      ...
      const combinedSolution = await config.combine(subproblem.data, childSolutions, depth);
