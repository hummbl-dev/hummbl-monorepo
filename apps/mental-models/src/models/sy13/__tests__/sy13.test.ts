...
import { createSY13Model } from '../index';
...
describe('SY13', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy13');
    expect(model.name).toBe('SY13 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY13Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY13Model({
