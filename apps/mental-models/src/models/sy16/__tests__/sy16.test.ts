...
import { createSY16Model } from '../index';
...
describe('SY16', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy16');
    expect(model.name).toBe('SY16 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY16Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY16Model({
