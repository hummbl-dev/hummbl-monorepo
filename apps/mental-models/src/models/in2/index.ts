...
export const createErrorUtilizationModel = (config = {}): ErrorUtilizationModel => {
  ...
  const generateImprovements = async ({
    ...
    for (const error of errors) {
      ...
      const existingImprovement = Array.from(improvements.values()).find((imp) =>
        imp.sourceErrors.includes(error.id)
      );
  ...
  const analyzeErrors = async (input: string | Record<string, any>): Promise<ErrorAnalysis> => {
    ...
    const avgImpact =
      generatedImprovements.length > 0
        ? generatedImprovements.reduce((sum, imp) => sum + imp.impact, 0) /
          generatedImprovements.length
        : 0;

    const avgEffort =
      generatedImprovements.length > 0
        ? generatedImprovements.reduce((sum, imp) => sum + imp.effort, 0) /
          generatedImprovements.length
        : 0;

    // Calculate error reduction potential (simplified)
    const errorReductionPotential = Math.min(
      1,
      generatedImprovements.length > 0
        ? generatedImprovements.reduce((sum, imp) => sum + imp.impact, 0) /
            generatedImprovements.length
...
export const analyzeErrors = async (
