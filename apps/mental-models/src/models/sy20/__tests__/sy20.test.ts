...
import { createSY20Model } from '../index';
...
describe('SY20', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy20');
    expect(model.name).toBe('SY20 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY20Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY20Model({
