...
import { createSY18Model } from '../index';
...
describe('SY18', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy18');
    expect(model.name).toBe('SY18 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY18Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY18Model({
