...
describe('P11', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP11Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p11');
    expect(defaultModel.name).toBe('P11 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP11Model({
