...
import { createSY17Model } from '../index';
...
describe('SY17', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy17');
    expect(model.name).toBe('SY17 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY17Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY17Model({
