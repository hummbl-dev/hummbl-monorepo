...
describe('P15', () => {
  ...
  it('should create a model with default config', () => {
    const defaultModel = createP15Model();
    
    expect(defaultModel).toBeDefined();
    expect(defaultModel.id).toBe('p15');
    expect(defaultModel.name).toBe('P15 Model');
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createP15Model({
