...
import { createSY10Model } from '../index';
...
describe('SY10', () => {
  ...
  it('should create a model with default config', () => {
    ...
    expect(model.id).toBe('sy10');
    expect(model.name).toBe('SY10 Synthesis Model');
  ...
  it('should handle confidence threshold', async () => {
    ...
    const highThresholdModel = createSY10Model({
      options: {
  ...
  it('should emit error events on failure', async () => {
    ...
    const failingModel = createSY10Model({
