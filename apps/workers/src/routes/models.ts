...
modelsRouter.get('/', async (c) => {
  ...
  try {
    let query = 'SELECT code, name, transformation, definition, whenToUse, example, priority FROM mental_models';
    const params: string[] = [];
...
modelsRouter.get('/:code', async (c) => {
  ...
  try {
    const model = await c.env.DB
      .prepare('SELECT code, name, transformation, definition, whenToUse, example, priority FROM mental_models WHERE code = ?')
      .bind(code)
      .first();
...
modelsRouter.get('/:code/relationships', async (c) => {
  ...
  try {
    ...
    const modelExists = await c.env.DB
