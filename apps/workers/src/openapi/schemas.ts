  ...
  .object({
    ...
    error: z.object({
      ...
      details: z
        .unknown()
        .optional()
        .describe('Additional error details (validation errors, etc.)'),
    }),
  })
  .openapi({
...
export const TransformationTypeSchema = z.enum(['P', 'IN', 'CO', 'DE', 'RE', 'SY']).openapi({
  description:
    'Transformation type: P=Perspective, IN=Input, CO=Connection, DE=Decision, RE=Reflection, SY=System',
  example: 'P',
});
...
export const MentalModelSchema = z
  .object({
    code: z
      .string()
      .regex(/^[A-Z]{1,2}\d{1,2}$/)
      .describe('Unique model code (e.g., P1, SY12)'),
...
export const ModelRelationshipSchema = z
  .object({
...
export const TransformationSchema = z
  .object({
    ...
    models: z
      .array(MentalModelSchema)
      .optional()
      .describe('Models belonging to this transformation'),
...
export const ModelFilterQuerySchema = z.object({
  ...
  search: z
    ...
    .max(100)
    .optional()
    .describe('Search term for model names and descriptions'),
  base_level: z.coerce
    .number()
    .int()
    .min(1)
    .max(5)
    .optional()
    .describe('Base difficulty level filter'),
});
...
export const RecommendRequestSchema = z
  ...
  .openapi({
    example: {
      problem:
        'I need to make a difficult business decision about whether to pivot our product strategy, but I have limited information and high uncertainty.',
    },
  });
...
export const GitHubAuthRequestSchema = z
  .object({
...
// User action schemas
export const AddProgressRequestSchema = z
  .object({
    modelId: z
      .string()
      .regex(/^[A-Z0-9]+$/)
      .describe('Model code to mark as completed'),
  })
  .openapi({
    example: {
      modelId: 'P1',
...
export const AddFavoriteRequestSchema = z
  .object({
    modelId: z
      .string()
      .regex(/^[A-Z0-9]+$/)
      .describe('Model code to add to favorites'),
  })
  .openapi({
    example: {
      modelId: 'P1',
